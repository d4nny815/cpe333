...
#						
#       Modified by James Mealy. This version of test_all.s
#       works with the univ_sseg module. If the program finds
#       an error, it outputs decimal 90zz to the 7-segment
#       display, where zz is the current test that 
#       generated the error. It pauses, then goes onto 
#       the next test, but does not complete the test where 
#       it encountered the error.  	
########################################################################

.equ test,     0x0044 
.equ SWITCHES, 0x11008000
.equ LEDS,     0x1100C000
.equ SSEG,     0x1100C00C
...
loop: 
	...
	jal x31, addi_test; call post_test	#10	16
	jal x31, andi_test; call post_test	#11	5
	jal x31, ori_test; call post_test	#12	5
	jal x31, xori_test; call post_test	#13	5
	jal x31, slli_test; call post_test	#14	15
	jal x31, srli_test; call post_test	#15	16
	jal x31, srai_test; call post_test	#16     16
	jal x31, sltiu_test; call post_test	#17     16
	jal x31, slti_test; call post_test	#18	16
	jal x31, lw_test; call post_test	#19	9
	jal x31, sw_test; call post_test	#20	9
	jal x31, beq_test; call post_test	#21	7
	jal x31, bne_test; call post_test	#22	7
	jal x31, blt_test; call post_test	#23	7
	jal x31, bge_test; call post_test	#24     10
	jal x31, bltu_test; call post_test	#25	7
	jal x31, bgeu_test; call post_test	#26	10
	jal x31, auipc_test; call post_test	#27	2
	jal x31, lui_test; call post_test	#28     5
	jal x31, jal_test; call post_test	#29	2
	jal x31, jalr_test; call post_test	#30	4
	jal x31, lh_test; call post_test	#31	10
	jal x31, lb_test; call post_test	#32	10
	jal x31, lhu_test; call post_test	#33	10
	jal x31, lbu_test; call post_test	#34	10
	jal x31, sh_test; call post_test	#35	10
	jal x31, sb_test; call post_test	#36	10
...
update_test:
	...
	ret
        ...
        # outputs decimal 90zz, where zz is the current test 
fail:   li    x13,0x00002328
        add   x13,x13,x14
        sw    x13,0(x11)



        # insert delay so you can see test number on 7-segment display
        li    x15,0x0000003F
loop1:  addi  x15,x15,-1
        bne   x15,x0,loop1

        # quit out of current test; move onto next test
